defmodule Thumbor.Support.HTTPSandbox do
  @moduledoc """
  ...
  In test `Thumbor.HTTP` will default to using this sandbox to get that function
  and run it, returning the response instead of hitting an external API.
  """
  @sleep 10
  @state "state"
  @disabled "disabled_pids"
  @registry :thumbor_http_sandbox
  @keys :unique

  @type action :: :get | :post | :patch | :put | :delete | :head
  @type url :: String.t()
  @type body :: map
  ...
  def get_response(url, headers, options) do
    ...
    case :erlang.fun_info(func)[:arity] do
      0 -> func.()
      1 -> func.(headers)
      2 -> func.(headers, options)
      _ ->
        raise """
        This function's signature is not supported.
        #{inspect(func)}
        Please provide a function that takes either no args or exactly three args.
        fn -> ... end
        fn (url, headers, options) -> ... end
        """
    end
  end
  ...
  @spec patch_response(url, body, headers, options) :: any
  def patch_response(url, body, headers, options) do
    func = find!(:patch, url)
  ...
  ```elixir
  Thumbor.Support.HTTPSandbox.set_get_responses([{"http://google.com/", fn ->
    {:ok, {"I am a response", %Thumbor.HTTP.Response{status: 200}}}
  end}])

  # the url headers and opts can be pattern matched here to assert the correct request was sent.
  Thumbor.Support.HTTPSandbox.set_get_responses([
  ...
  @spec set_patch_responses([{String.t(), fun}]) :: :ok
  def set_patch_responses(tuples) do
    tuples
    |> Map.new(fn {url, func} -> {{:patch, url}, func} end)
    |> then(&SandboxRegistry.register(@registry, @state, &1, @keys))
    |> then(fn
      :ok -> :ok
      {:error, :registry_not_started} -> raise_not_started!()
  ...
  import Thumbor.Support.HTTPSandbox, only: [disable_http_sandbox: 1]
  ...
  defp find_response!(funcs, action, url) do
    ...
    with funcs when is_map(funcs) <- Map.get(funcs, key, funcs),
         regexes <- Enum.filter(funcs, fn {{_, k}, _v} -> is_regex?(k) end),
         {_regex, func} when is_function(func) <-
           Enum.find(regexes, funcs, fn {{key, regex}, _v} -> Regex.match?(regex, url) and (key === action) end) do
      func
    else
  ...
  defp format_example(action, url) do
    """
    alias Thumbor.Support.HTTPSandbox
